...
namespace esphome
{
    namespace idasen_desk_controller
    {
        ...
        static void deskHeightUpdateNotificationCallback(BLERemoteCharacteristic *pBLERemoteCharacteristic, uint8_t *pData, size_t length, bool isNotify)
        {
        ...
        void IdasenDeskControllerComponent::set_mac_address(uint64_t address)
        {
            ...
            snprintf(buffer, sizeof(buffer), "%02X:%02X:%02X:%02X:%02X:%02X", address8[5], address8[4], address8[3], address8[2], address8[1], address8[0]);
        ...
        void IdasenDeskControllerComponent::connect()
        {
            ...
            {
                this->m_control_char_ = this->p_client_->getService(controlServiceUUID)->getCharacteristic(controlCharacteristicUUID);
            }
        ...
        void IdasenDeskControllerComponent::onConnect(BLEClient *p_client)
        {
            this->set_connection_(true);
        }
        ...
        void IdasenDeskControllerComponent::update_desk_()
        {
            ...
            unsigned short height = this->get_height_();
            if ((this->move_up_ && height >= this->height_target_) || (false == this->move_up_ and height <= this->height_target_))
            {
        ...
        unsigned short IdasenDeskControllerComponent::get_height_()
        {
            return this->m_output_char_->readUInt16() / 100;
        }
